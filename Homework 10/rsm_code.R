...
mcoal_code <- lm(emissions~A_coded+B_coded+
                   A_coded:B_coded,
data=coal)
anova(mcoal_code)
summary(mcoal_code)
...
mcode_coal <- lm(emissions~A_coded+B_coded+
                   A_coded:B_coded,data=coal)
...
pred_data <- data.frame(poly_code=1,prop_code=-1,
                        sucrose_code=-1)
# ^^^ using the values for the lowest turbidity (desired effect) ^^^ #

#use the predict function to obtain standard error
check2 <- predict(m2,pred_data,se=TRUE,interval="confidence")
check2
# ^^^ uses m2 which is our best-model, with removed terms ^^^ #
